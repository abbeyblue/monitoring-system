...
export default function AlertsPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Alerts" sub="Active and recent alerts. Symptoms are correlated into incidents so one failure doesn't page ten times." crumbs={[{ label: "Incidents", href: "/incidents" }, { label: "Alerts" }]} />
        ...
        <div className="divide-y divide-line">
          {ALERTS.map((a) => (
            <div key={a.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2.5 text-xs">
              ...
              {a.runbook && <a href={`/runbooks#${a.runbook}`} className="rounded border border-line px-1.5 py-0.5 text-2xs text-muted hover:text-ink cursor-pointer">runbook</a>}
              {a.incident && <a href={`/incidents/${a.incident}`} className="rounded border border-down/40 px-1.5 py-0.5 text-2xs text-down cursor-pointer">{a.incident}</a>}
