  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title={inc.title} sub={`${inc.id} · started ${inc.start} · ${inc.duration} · ${inc.impact}`}
        crumbs={[{ label: "Incidents", href: "/incidents" }, { label: inc.id }]}
      ...
      <div className="mb-3">
        <Related items={[
          ...
          ...(inc.trace ? [{ label: `Trace ${inc.trace}`, href: `/traces/${inc.trace}` }] : []),
          { label: "Database", href: "/databases/crm-prod-db" },
          { label: "Logs", href: "/logs" },
          ...(inc.runbook ? [{ label: `Runbook`, href: `/runbooks#${inc.runbook}` }] : []),
        ]} />
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
        <Panel title="Correlated alerts" count={`${alerts.length}`}>
          <div className="divide-y divide-line">
            {alerts.map((a) => (
              <a key={a.id} href="/alerts" className="flex items-center gap-2 px-4 py-2.5 text-xs transition-colors hover:bg-panel2 cursor-pointer">
