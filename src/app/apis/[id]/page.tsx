  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title={`${a.method} ${a.endpoint}`} sub={`${svc?.name} · ${a.version} · owner ${a.owner} · last deploy ${a.lastDeploy}`}
        crumbs={[{ label: "APIs", href: "/apis" }, { label: a.endpoint }]} right={<Pill health={a.health} />} />

      <div className="mb-3">
        <Related items={[
          ...(svc ? [{ label: svc.name, href: `/service-health/${svc.id}` }] : []),
          ...(trace ? [{ label: `Trace ${trace.id}`, href: `/traces/${trace.id}` }] : []),
          ...(incident ? [{ label: `Incident ${incident.id}`, href: `/incidents/${incident.id}` }] : []),
          ...(deploy ? [{ label: `Deploy ${deploy.version}`, href: `/deployments/${deploy.id}` }] : []),
                ...
                {errs.map((e) => (
                  <tr key={e.code} className="border-b border-line/60 last:border-0">
                    ...
                    <td className="px-3 py-2">
                      {e.incident && <a href={`/incidents/${e.incident}`} className="mr-1 rounded border border-down/40 px-1 text-2xs text-down hover:bg-down/10 cursor-pointer">{e.incident}</a>}
