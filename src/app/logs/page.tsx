...
export default function LogsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Log Explorer" sub="Structured CloudWatch + application logs. Filter by level, service, or search by request/trace id — every row links into its trace." crumbs={[{ label: "Observability", href: "/" }, { label: "Logs" }]} />
        ...
        <div className="divide-y divide-line font-mono text-2xs">
          {rows.map((l, i) => (
            <div key={i} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2 transition-colors hover:bg-panel2">
              ...
              {l.traceId !== "-" && <a href={`/traces/${l.traceId}`} className="text-accent hover:underline cursor-pointer">{l.traceId}</a>}
