...
export default function AuditLogsPage() {
  return (
    <div className="mx-auto max-w-[1200px]">
      <PageHead mock title="Audit Logs" sub="Immutable-style trail of significant actions — who did what, when, from where, with the request id." crumbs={[{ label: "Security", href: "/" }, { label: "Audit Logs" }]} />
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-xs">
          ...
          <tbody>{AUDIT_LOGS.map((a) => (
            <tr key={a.id} className="border-b border-line/60 last:border-0">
              ...
              <td className="px-3 py-2"><a href={`/logs?q=${a.requestId}`} className="font-mono text-2xs text-accent hover:underline cursor-pointer">{a.requestId}</a></td>
            </tr>
          ))}</tbody>
        </table></div>
      </Panel>
