...
export default function TracesPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Traces" sub="Distributed traces across CloudFront → API Gateway → service → cache → database → queue → worker. Open a trace for the span waterfall." crumbs={[{ label: "Observability", href: "/" }, { label: "Traces" }]} />
            ...
            <tbody>
              {TRACES.map((t) => (
                <tr key={t.id} className="border-b border-line/60 transition-colors last:border-0 hover:bg-panel2">
                  <td className="px-3 py-2"><Link href={`/traces/${t.id}`} className="flex items-center gap-2 font-mono text-2xs cursor-pointer"><Dot health={t.health} />{t.id}</Link></td>
