...
export default function MetricsPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Metrics — Golden Signals" sub="Traffic, latency, errors, and saturation for every service (RED + USE)." crumbs={[{ label: "Services", href: "/service-health" }, { label: "Metrics" }]} />
      <div className="space-y-3">
        {SERVICES.map((s, idx) => (
          <Panel key={s.id} title={s.name} status={s.health} right={<a href={`/service-health/${s.id}`} className="text-2xs text-accent hover:underline cursor-pointer">details →</a>}>
