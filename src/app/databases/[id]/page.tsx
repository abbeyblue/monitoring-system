  ...
  if (!isPrimary) {
    return (
      <div className="mx-auto max-w-[1400px]">
        <PageHead mock title={meta?.name ?? id} sub={meta?.engine} crumbs={[{ label: "Databases", href: "/databases" }, { label: meta?.name ?? id }]} right={meta && <Pill health={meta.health} />} />
        <Panel title="Telemetry" pad><p className="text-xs text-muted">Full instance telemetry is wired for <span className="font-mono">crm-prod-db</span> in this prototype. <a href="/databases/crm-prod-db" className="text-accent hover:underline cursor-pointer">Open crm-prod-db →</a></p></Panel>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead title={DB.name} sub={`${DB.engine} ${DB.version} · ${DB.instance} · ${DB.role} · ${DB.region}`}
        crumbs={[{ label: "Databases", href: "/databases" }, { label: DB.name }]} right={<Pill health={DB.health} />} />

      <div className="mb-3">
        <Related items={[
          { label: "Incident INC-1042", href: "/incidents/INC-1042" },
          { label: "Trace trc_9f2a71c4e8", href: "/traces/trc_9f2a71c4e8" },
          { label: "Deploy v1.42.3", href: "/deployments/dpl_442" },
          { label: "Customer Service", href: "/service-health/customer" },
        ]} />
      </div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
        <Panel title="Active alerts" count={`${dbAlerts.length}`} status="critical">
          <div className="divide-y divide-line">
            {dbAlerts.map((a) => (
              <a key={a.id} href="/alerts" className="flex items-center gap-2 px-4 py-2 text-xs transition-colors hover:bg-panel2 cursor-pointer">
