...
export default function DatabasesPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Databases" sub="CRM databases with CPU, storage, and connection saturation. Open one for the full dashboard." crumbs={[{ label: "Services", href: "/service-health" }, { label: "Databases" }]} />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {DATABASES.map((d) => (
          <Link key={d.id} href={`/databases/${d.id}`} className="block cursor-pointer">
