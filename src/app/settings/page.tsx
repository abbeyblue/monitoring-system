...
import { Panel, Sq, PageHead, COLOR, fmtClock } from "@/components/ui";
...
export default function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1100px]">
      <PageHead title="Settings" sub="Environment status, alert thresholds (saved to the datastore), and silences." />
...
function EnvRow({ ok, label, on, off, warnWhenOff }: { ok?: boolean; label: string; on: string; off: string; warnWhenOff?: boolean }) {
  const health = ok ? "ok" : warnWhenOff ? "warn" : "idle";
  return (
    <div className="flex items-center gap-2 px-4 py-2.5 text-xs">
      <Sq health={health} />
      <span className="font-medium">{label}</span>
      <span className="ml-auto text-[11px]" style={{ color: COLOR[health] }}>{ok ? on : off}</span>
    </div>
  );
}
