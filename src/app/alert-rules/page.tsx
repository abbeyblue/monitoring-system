...
export default function AlertRulesPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Alert Rules" sub="Threshold and condition rules across APIs, database, queues, Lambda, and backups." crumbs={[{ label: "Incidents", href: "/incidents" }, { label: "Alert Rules" }]} />
        ...
        <div className="divide-y divide-line">
          {RULES.map((r) => (
            <div key={r.id} className="flex items-center gap-3 px-4 py-2.5 text-xs">
              ...
              {r.fires && <a href="/alerts" className="rounded border border-line px-1.5 py-0.5 text-2xs text-muted hover:text-ink cursor-pointer">{r.fires}</a>}
