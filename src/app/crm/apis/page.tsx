...
export default function ApisPage() {
  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead title="APIs" sub="Live server-side probes of the real CRM endpoints, plus the sample inventory with full latency percentiles." crumbs={[{ label: "Services", href: "/crm/service-health" }, { label: "APIs" }]} />
              ...
              <tbody>
                {APIS.map((a) => (
                  <tr key={a.id} className="border-b border-line/60 transition-colors last:border-0 hover:bg-panel2">
                    <td className="px-3 py-2"><Link href={`/crm/apis/${a.id}`} className="flex items-center gap-2 cursor-pointer"><Dot health={a.health} /><span className="font-mono text-2xs font-semibold" style={{ color: a.method === "POST" ? "rgb(var(--accent))" : "rgb(var(--muted))" }}>{a.method}</span><span className="font-medium">{a.endpoint}</span></Link></td>
