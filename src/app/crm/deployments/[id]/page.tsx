  ...
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title={`${d.version}`} sub={`${d.commit} · ${d.env} · deployed by ${d.deployer} · ${d.start}–${d.end}`}
        crumbs={[{ label: "Deployments", href: "/crm/deployments" }, { label: d.version }]}
      ...
      <div className="mb-3">
        <Related items={[
          { label: `Services: ${d.services.join(", ")}`, href: "/crm/service-health" },
          ...(incident ? [{ label: `Incident ${incident.id}`, href: `/crm/incidents/${incident.id}` }] : []),
          ...(incident?.trace ? [{ label: `Trace ${incident.trace}`, href: `/crm/traces/${incident.trace}` }] : []),
