...
export default function DeploymentsPage() {
  return (
    <div className="mx-auto max-w-[1400px]">
      <PageHead mock title="Deployments" sub="Every release correlated with error rate, latency, and resource deltas against the prior version." crumbs={[{ label: "Incidents", href: "/crm/incidents" }, { label: "Deployments" }]} />
            ...
            <tbody>
              {DEPLOYMENTS.map((d) => (
                <tr key={d.id} className="border-b border-line/60 transition-colors last:border-0 hover:bg-panel2">
                  <td className="px-3 py-2"><Link href={`/crm/deployments/${d.id}`} className="flex items-center gap-2 cursor-pointer"><Dot health={d.status} /><span className="font-medium">{d.version}</span><span className="font-mono text-2xs text-muted">{d.commit}</span>{d.rollback && <span className="rounded border border-down/40 px-1 text-[10px] text-down">rollback</span>}</Link></td>
                  ...
                  <td className="px-3 py-2">{d.incident ? <a href={`/crm/incidents/${d.incident}`} className="rounded border border-down/40 px-1 text-2xs text-down cursor-pointer">{d.incident}</a> : <span className="text-2xs text-ok">clean</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
